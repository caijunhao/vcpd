...
custom_parameters = [{"name": "--headless", 'type': int, "default": 1, "help": "Direct 1, GUI 0"},
                     {"name": "--num_envs", "type": int, "default": 2, "help": "Number of environments to create"},
                     {"name": "--num_objects", "type": int, "default": 30, "help": "Number of objects in the bin"},
                     ...
                     {"name": "--model_used", "type": str, "default": "cpn", "help": "cpn,vpn,vgn"},
...
# paths = np.random.choice(os.listdir(asset_root), 100, replace=False)
paths = os.listdir(asset_root)
asset_paths = []
for path in paths:
    asset_paths.append(path + '/' + path + '.urdf')
...
for i in range(num_envs):
    ...
    # create bin
    # bin_pose.p = gymapi.Vec3(0, 0, asset_options.thickness/2)
...
while True:
    ...
    # if time['up'] < t <= time['away']:
    #     rigid_obj = torch.clone(root_tensor)
    #     rigid_obj[panda_root_idxs, 1] += 0.001 * d_
    #     actor_indices = torch.tensor([panda_root_idxs], dtype=torch.int32, device=device)
    #     gym.set_actor_root_state_tensor_indexed(sim, gymtorch.unwrap_tensor(rigid_obj),
    #                                             gymtorch.unwrap_tensor(actor_indices),
    #                                             num_envs)

    # if time['away'] < t <= time['shake_4']:
    #     if t < time['shake_1'] or time['shake_2'] < t < time['shake_3']:
    #         angle = 2
    #     else:
    #         angle = -2
    #     rigid_obj = torch.clone(root_tensor)
    #     hand_quat = rigid_obj[panda_root_idxs, 3:7].cpu().numpy()
    #
    ...
    #     rigid_obj[panda_root_idxs, 3:7] = hand_quat.to(torch.float32)
    #     actor_indices = torch.tensor([panda_root_idxs], dtype=torch.int32, device=device)
    #     gym.set_actor_root_state_tensor_indexed(sim, gymtorch.unwrap_tensor(rigid_obj),
    #                                             gymtorch.unwrap_tensor(actor_indices),
    #                                             num_envs)
...
np.savetxt('../log/%s_%s'%(num_objects, args.idx),txt,fmt='%i', delimiter=",")
if not args.headless:
