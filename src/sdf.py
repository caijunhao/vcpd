...
class SDF(object):
    def __init__(self, volume_bounds, resolution, rgb=False, device='cuda', dtype=torch.float32):
        ...
        self.vol_dims = np.ceil((self.vol_bnd[:, 1] - self.vol_bnd[:, 0]) / self.res).astype(np.int)
        self.vol_bnd[:, 1] = self.vol_bnd[:, 0] + self.vol_dims * self.res
        self.origin = torch.from_numpy(self.vol_bnd[:, 0]).float().view(1, 3).to(device)

        # Initialize signed distance and weight volumes
        self.sdf_vol = torch.ones(self.vol_dims.tolist(), dtype=self.dtype).to(device)
        self.w_vol = torch.zeros_like(self.sdf_vol, dtype=self.dtype).to(device)
        # record if the voxel has been scan at least once
        self.valid = torch.zeros_like(self.sdf_vol, dtype=torch.bool).to(device)
        if self.rgb:
            self.rgb_volume = torch.ones_like(self.sdf_vol, dtype=self.dtype).to(device)
    ...
    def sdf_integrate(self, depth, intrinsic, camera_pose, weight=1.0, rgb=None):
        ...
        :return: None
        """
        depth = torch.from_numpy(depth).to(self.dev)
        weight = torch.tensor(weight, device=self.dev)
        height, width = depth.shape
        c2w = torch.from_numpy(camera_pose).to(self.dev)
        w2c = torch.inverse(c2w)
        intrinsic = torch.from_numpy(intrinsic).to(self.dev)
        # inv_intrinsic = torch.inverse(intrinsic)
        world_positions = torch.cat([self.world_pos,
                                     torch.ones(self.vol_dims.tolist() + [1], dtype=self.dtype).to(self.dev)],
                                    dim=-1)
        ...
        diff_depth = torch.zeros_like(depth_volume, dtype=self.dtype, device=self.dev)
        diff_depth[valid_depth] = depth_volume[valid_depth] - camera_positions[valid_voxels][valid_depth][:, 2]
        distances = diff_depth / (self.res * 5.0)  # rescale the distance
        self.valid[valid_voxels] = valid_depth
        valid_x = self.voxel_coords[valid_voxels][valid_depth][:, 0]
        valid_y = self.voxel_coords[valid_voxels][valid_depth][:, 1]
        valid_z = self.voxel_coords[valid_voxels][valid_depth][:, 2]
        ...
        if self.rgb and rgb is not None:
            ...
            rgb = torch.from_numpy(rgb).to(self.dev)
            valid_color = rgb[
            ...
            rgb_new = torch.minimum(rgb_new, torch.tensor(255.0, device=self.dev))
            self.rgb_volume[valid_x, valid_y, valid_z] = self.encode_rgb(rgb_new)

    def reset(self):
        self.sdf_vol = torch.ones(self.vol_dims.tolist(), dtype=self.dtype).to(self.dev)
        self.w_vol = torch.zeros_like(self.sdf_vol, dtype=self.dtype).to(self.dev)
        if self.rgb:
            self.rgb_volume = torch.ones_like(self.sdf_vol, dtype=self.dtype).to(self.dev)
            self.rgb_volume = self.rgb_volume * (255 * 256 ** 2 + 255 * 256 + 255)
    ...
    def compute_pcl(self, threshold=0.2, use_post_processed=True, gaussian_blur=True):
        ...
        xyz = self.world_pos[valid_voxels]
        if self.rgb:
    ...
    def compute_mesh(self, use_post_processed=True, gaussian_blur=True, step_size=3):
        ...
        v = v * self.res + self.vol_bnd[:, 0:1].T
        if self.rgb:
    ...
    def sdf_info(self):
        print('volume bounds: ')
        print('x_min: {:04f}m'.format(self.vol_bnd[0, 0]))
        print('x_max: {:04f}m'.format(self.vol_bnd[0, 1]))
        print('y_min: {:04f}m'.format(self.vol_bnd[1, 0]))
        print('y_max: {:04f}m'.format(self.vol_bnd[1, 1]))
        print('z_min: {:04f}m'.format(self.vol_bnd[2, 0]))
        print('z_max: {:04f}m'.format(self.vol_bnd[2, 1]))
...
class TSDF(SDF):
    def __init__(self, volume_bounds, resolution, rgb=False, device='cuda', dtype=torch.float32):
        ...
        self.auxiliary_sdf = self.sdf_vol.detach().clone()
        self.auxiliary_w = self.w_vol.detach().clone()
    ...
    def tsdf_integrate(self, depth, intrinsic, camera_pose, weight=1.0, rgb=None):
        ...
        w2c = torch.inverse(c2w)
        intrinsic = torch.from_numpy(intrinsic).to(self.dev)
        # normal = self.compute_surface_normal_least_square(depth, intrinsic, dev=self.dev)
        ...
        valid_x = self.voxel_coords[valid_voxels][valid_depth_tsdf][:, 0]
        valid_y = self.voxel_coords[valid_voxels][valid_depth_tsdf][:, 1]
        valid_z = self.voxel_coords[valid_voxels][valid_depth_tsdf][:, 2]
        w_old = self.w_vol[valid_x, valid_y, valid_z]
        sdf_old = self.sdf_vol[valid_x, valid_y, valid_z]
        ...
        if self.rgb and rgb is not None:
            ...
            # rgb_new = (torch.unsqueeze(w_old, dim=1) * rgb_old + weight * valid_color) / torch.unsqueeze(w_new, dim=1)
            rgb_new = valid_color
            # rgb_new = torch.minimum(rgb_new, torch.tensor(255.0, dev=self.dev))
            self.rgb_volume[valid_x, valid_y, valid_z] = self.encode_rgb(rgb_new)
    ...
    def post_processed_volume(self):
        ...
        sdf_volume = self.sdf_vol.clone()
        in_obj_ids = (self.w_vol == 0) * (self.auxiliary_sdf < 0)
